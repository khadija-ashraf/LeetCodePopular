...
//                printState(i, nums);
    ...
    // whenever heap size changes call this to balance the size  of heaps
    // and heap size changes when we add/insert, or when we remove/poll
    // in both cases we call the balanceHeaps(). Thats why we call this method 
    // two time in this code.
    private void balanceHeaps() {
        if (maxHeap.size() > minHeap.size() + 1) {
            minHeap.offer(maxHeap.poll());
            prune(maxHeap);
//          Why not prune(minHeap)?
//    		Because:
//    			•	The delayed element was in maxHeap
//    			•	We just pulled from maxHeap
//    			•	So maxHeap’s new top might now be another invalid value
//    			•	We must check and clean that new top
...
//    we always call 'prune()':
//    at key points:
//    	•	After inserting new elements
//    	•	After window range exceeds 
//    	•	After moving between heaps/ balancing
